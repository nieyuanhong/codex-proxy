...
export class CodexApi {
  ...
  private buildConversationIdentity(request: CodexResponsesRequest): {
    ...
    const conversationId = clientConversationId
      ...
      : null;
    const clientWindowId = firstCodexRequestString(request, X_CODEX_WINDOW_ID_HEADER);
    return {
      conversationId,
  ...
  private async createResponseViaWebSocket(
    ...
    wsRequest.client_metadata = buildCodexClientMetadata(request, installationId, identity.windowId);
  ...
  private async createResponseViaHttp(
    ...
    const bodyWithMetadata = {
      ...
      client_metadata: buildCodexClientMetadata(request, installationId, identity.windowId),
    };
