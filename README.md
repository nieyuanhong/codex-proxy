...
<details>
<summary><h3>方式二：No-Node Lite（浏览器/服务器版，高级用户）</h3></summary>

如果你已经安装 Node.js，或者只想在没有图形桌面的机器上运行服务，可以选择这个额外的 Lite 版本。上面的 Electron 安装包不受影响；Lite 使用同一套服务和控制面板，但不内置 Node.js，因此下载包更小，也方便你自行管理运行时。

Lite 发布包是 `codex-proxy-<版本>-no-node-lite-all-platforms.zip`。下载并解压后，在包根目录启动：

```bash
# Windows：双击 codex-proxy.exe
# macOS/Linux：
chmod +x codex-proxy.sh
./codex-proxy.sh
```

使用前请准备 Node.js 20 或更新版本。Windows 启动器会优先尝试使用 WebView2 打开控制面板；如果 WebView2 不可用，就启动服务并在系统浏览器中打开实际监听地址。macOS/Linux 默认使用系统浏览器。Windows 的 `codex-proxy.cmd` 和其他平台的命令行入口可以在终端中使用，便于查看日志和排查问题。

常用启动选项：

- `--mode=server`（`-m server`）：只启动服务，不打开窗口或浏览器。
- `--mode=browser`：使用系统浏览器打开控制面板。
- `--mode=webview2`：Windows 上强制使用 WebView2；不可用时会提示处理方式。
- `--host`（`-H`）、`--port`（`-P`）：指定监听地址和端口。
- `--node-path`（`-n`）：指定要使用的 Node.js 可执行文件。
- `--portable`（`-p`）：把配置和数据保存到 Lite 包目录下；不指定时，行为与 Electron 版一样，使用当前用户的数据目录。

Lite 不会静默安装 Node.js。找不到 Node.js 时，启动器会显示安装指引，并在等待确认后退出。显式要求 WebView2、但电脑尚未安装 WebView2 Runtime 时，程序会先询问是否从 Microsoft 在线下载并运行安装程序；需要联网，且不会把安装程序预先放进 Lite 压缩包。Lite 的“检查更新”会打开最新 Releases 页面，不会自动覆盖正在运行的目录。

Linux x64 Lite 同时适用于常见的 glibc Linux 和 Alpine 等 musl Linux；这两类系统都仍需安装兼容的 Node.js。
