...
[English](./API.md) | [简体中文](./API_CN.md) | [繁體中文 (台灣)](./API_TW.md) | **繁體中文 (香港)** | [日本語](./API_JA.md)
...
### 客戶端子密鑰（Client Keys / Sub-keys）
客戶端亦可使用在後台管理面板或 Admin API 建立的細粒度 Client Key 進行認證。子密鑰支援配額上限（USD 預算）、Token 上限、並發上限、允許訪問的模型列表以及過期時間。
- 子密鑰自我查詢端點：`GET /v1/sub-key/info`（需要傳入 `Authorization: Bearer {client_key}`）。
...
- `max_tokens`、`max_completion_tokens`、`max_output_tokens` 僅作客戶端相容解析，不會轉發至 Codex 原生後端。
...
- 認證：`x-api-key` 或 `Authorization: Bearer`
...
- 認證：`x-goog-api-key` 請求標頭、`key` 查詢參數、或 Bearer token
...
- 請勿向原生 Codex 發送 `max_output_tokens`。代理僅作相容解析並予以剔除，因為真實 Codex 後端會返回 `400 Unsupported parameter: max_output_tokens` 錯誤。

### WebSocket /v1/responses
原生 Codex Responses API WebSocket 傳輸（Issue #681）。

客戶端可透過 WebSocket 連接至 `ws://{host}:{port}/v1/responses`（或 `wss://`），並攜帶標準認證資訊（如 `Authorization: Bearer {key}` 標頭或 `?key={key}` 查詢參數）。

- 連接維持長連，支援多輪對話互動。
- 客戶端發送 `response.create` 格式的 JSON 文本幀。
- 代理執行請求並將 SSE 的 `data:` JSON 負載逐幀返回給客戶端。
...
- 返回 OpenAI 相容格式 `{ created, data: [{ b64_json, revised_prompt }] }`。
...
所有端點均要求 body 含非空 `model`，並使用現有模型路由選擇 API-key entry。代理以供應商 API Key 替換本地代理認證；除套用已設定的模型別名/內部 provider 前綴解析外，不改寫 JSON body，並原樣返回上游狀態碼、Content-Type 與回應主體。未列入白名單的路徑不會被轉發。亦接受不帶 `/v1` 的本地別名。遠端壓縮的公開合約見 [OpenAI Responses compact API](https://developers.openai.com/api/reference/resources/responses/methods/compact/)；獨立搜尋路徑來自 [Codex CLI 0.147.0 官方源代碼](https://github.com/openai/codex/blob/rust-v0.147.0/codex-rs/codex-api/src/endpoint/search.rs#L31-L45)。

#### image_generation 工具

在 `tools[]` 內宣告 `{"type": "image_generation", ...}`，模型即可調用伺服器端圖像生成後端（`gpt-image-2`）。前提：**ChatGPT Plus 及以上** 帳號——free 帳號上游會靜默剝除工具，模型會改用 SVG 文本替代繪圖。

**支援欄位**（除 `type` 外全部為可選）：

| 欄位 | 列舉 / 範圍 | 預設值 | 備註 |
|---|---|---|---|
| `size` | `1024x1024`、`1024x1536`、`1536x1024`、`2048x2048`、`2048x3072`、`3072x2048`、`3840x2160`（4K UHD）、`2160x3840`（4K 直向）、`2304x3072`（3:4）、`auto` | `auto` | 寬高必須均為 16 的倍數；最長邊 ≤ 3840 px；總像素預算約 8 MP（`3072x3072` 會被拒絕）；低於 1024 px 解像度亦會被拒（最小像素預算）|
...
- `model` — 無論傳入甚麼，上游強制改回 `gpt-image-2`（回應回顯為 `gpt-image-2-codex`）。
- `size` — 客戶端請求的 `2048x2048`、`2K`、`4K` 等尺寸會被上游回顯/正規化為 `auto`，實際輸出解像度由伺服器端自行決定（例如實測 `1254x1254`）。
- `quality` — 傳入任何值均被 echo 為 `auto`，用戶自訂值不生效。
- `n` — `unknown_parameter`；一次只能生成一張圖片。
...
**Token 計費**：`response.completed.response.usage` 為主模型的 token；圖像工具的 token 單獨透過 `response.completed.response.tool_usage.image_gen.{input_tokens, output_tokens, total_tokens}` 返回。代理兩端均原樣直通，並在控制面板中將圖像 token 單列為 `total_image_input_tokens` / `total_image_output_tokens`，不會與主模型的 token 混淆。

**請求計數**：代理同時分別統計圖像生成的成功 / 失敗次數。`total_image_request_count` 在上游返回真實圖像（`tool_usage.image_gen.output_tokens > 0`）時 +1；`total_image_request_failed_count` 在工具被靜默剝除（Free 帳號）、上游錯誤、空回應等任何失敗路徑下 +1。兩者皆透過 `/admin/usage-stats/summary` 公開，Dashboard 的「Image Requests」卡片直接展示 `N ok · M failed`。
...
OpenAI Chat 相容路徑會接受 `tools: [{"type":"image_generation"}]`，但穩定的圖像 payload 僅會透過 `/v1/responses` 的 `image_generation_call.result` 暴露。需要獲取 base64 圖片字節時，請使用 `/v1/responses` 或 `POST /v1/images/generations`。

---

### Ollama 相容橋接

可選橋接服務運行於獨立的監聽端口上，預設為 `http://127.0.0.1:11434`。預設處於關閉狀態，可透過控制面板或 Admin API 開啟。Ollama 端點設計為免認證，除非您信任當前網絡環境，否則建議僅綁定在本地回環地址（localhost）。

瀏覽器 CORS 訪問被限制在本地回環來源（`localhost`、`127.x.x.x`、`::1`），非本地網頁預設無法讀取橋接回應。橋接層在轉發 `/v1/*` 請求時會自動注入設定好的 Codex Proxy API Key，因此向局域網或公網暴露該端口等同於免密碼暴露代理主介面。

| 方法 | 路徑 | 說明 |
|--------|------|-------------|
| GET | `/api/version` | 版本探測 → `{ version }` |
| GET | `/api/tags` | Ollama 格式的模型列表 |
| POST | `/api/show` | 模型元數據與能力描述 |
...
| GET | `/v1/models/catalog` | 完整模型目錄（含 reasoning effort 及元數據） |
| GET | `/v1/models/:id` | 單一模型詳細資訊 |
| GET | `/v1/models/:id/info` | 擴展模型資訊 |
| GET | `/v1beta/models` | 列出模型（Gemini 格式） |
| POST | `/admin/refresh-models` | 強制從上游刷新模型列表 |

模型目錄條目可以包含 token 元數據：

| 欄位 | 含義 |
|------|------|
| `contextWindow` | 靜態或上游提供的上下文視窗，用於展示和客戶端參考 |
| `maxContextWindow` | 上游提供的最大可擴展上下文視窗（若有返回） |
| `maxOutputTokens` | 靜態或上游提供的最大輸出 token，用於展示和客戶端參考 |
| `truncationPolicyLimit` | 上游提供的截斷策略限制（若有返回） |
...
## 客戶端子密鑰管理（Client Keys）

透過子密鑰可以生成帶有特定額度、模型權限、Token 限制、並發控制和過期時間的獨立 API Key。
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/admin/client-keys` | 列出所有 Client Keys（遮罩處理）及匯總統計 |
...
| POST | `/admin/client-keys/:id/toggle` | 快速啟用 / 停用密鑰（`active` / `disabled`） |
| POST | `/admin/client-keys/:id/reset-usage` | 重設指定密鑰的用量花費與 Token 計數 |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| POST | `/auth/accounts/import` | 批量導入（`{ accounts: [{token?, refreshToken?, label?}] }` 或純文字格式） |
| POST | `/auth/accounts/batch-delete` | 批量刪除（`{ ids: [] }`） |
| POST | `/auth/accounts/batch-status` | 批量啟用/停用（`{ ids: [], status: "active" | "disabled" }`） |
...
| POST | `/auth/accounts/:id/refresh` | 刷新單一帳號 token 和狀態 |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/auth/accounts/export` | 導出帳號（`?ids=a,b&format=minimal|full|csv|token-key|auth-json|sub2api`） |
...
| POST | `/auth/api-keys/models` | 從上游供應商拉取可用模型列表 |
| GET | `/auth/api-keys/export` | 導出 API Keys 以便重新導入 |
| POST | `/auth/api-keys/import` | 批量導入 Provider API Keys（`{ keys: [] }`） |
| POST | `/auth/api-keys` | 新增單一 Provider Key 綁定（`{ provider, models, apiKey, baseUrl?, label?, capabilities?, wire? }`） |
| POST | `/auth/api-keys/batch-delete` | 批量刪除 API Keys（`{ ids: [] }`） |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| POST | `/auth/login-start` | 發起 OAuth → 返回 `{ authUrl, state }` |
| GET | `/auth/login` | 302 重定向至 Auth0 |
...
| POST | `/auth/import-cli` | 從 Codex CLI auth.json 導入 |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/api/proxies/export` | 導出為 YAML |
| POST | `/api/proxies/import` | 導入 YAML 或純文字（`host:port:user:pass` 格式） |
| GET | `/api/proxies/assignments/export` | 導出分配關係 |
| POST | `/api/proxies/assignments/import` | 預覽分配導入（不執行） |
| POST | `/api/proxies/assignments/apply` | 套用分配導入 |
...
| POST | `/admin/general-settings` | 更新設定（返回 `restart_required` 標誌） |
| GET | `/admin/settings` | 取得 Master proxy API key |
| POST | `/admin/settings` | 設定 Master proxy API key |
| GET | `/admin/rotation-settings` | 取得輪換策略 |
| POST | `/admin/rotation-settings` | 設定輪換策略（`least_used` | `round_robin` | `sticky`） |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/admin/logs` | 查詢捕獲的請求日誌列表（`?limit=&offset=&direction=&search=`） |
| GET | `/admin/logs/state` | 取得請求日誌存儲狀態（`enabled`, `paused`, `capacity`） |
| POST | `/admin/logs/state` | 更新日誌捕獲狀態（`{ enabled?, paused? }`） |
...
| 方法 | 路徑 | 說明 |
|------|------|------|
| GET | `/admin/error-logs` | 取得匯總後的錯誤日誌列表 |
| GET | `/admin/error-logs/raw` | 取得原始錯誤日誌列表（`?limit=`） |
...
| POST | `/official-agent/threads/:threadId/turns` | 發起 turn，並以 SSE 串流返回 app-server notifications |
...
啟用 `quota.skip_exhausted` 後，帳號池會在取得帳號時過濾快取額度中 `rate_limit.limit_reached === true`、`secondary_rate_limit.limit_reached === true` 或 `code_review_rate_limit.limit_reached === true` 的 active 帳號。過濾發生在 session affinity 之前，所以 `preferredEntryId` 不能將請求繼續粘附在已耗盡帳號。若只是 `used_percent=99` 這類逼近上限但上游尚未標記 `limit_reached` 的情況，代理不會主動跳過；等上游返回 429 後，該帳號會進入 `rate_limited` 退避並自動切換至其他可用帳號。
